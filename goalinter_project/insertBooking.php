...
if (isset($_GET)) {
	if ($_GET['isAdd'] == 'true') {
        ...
        //     }
        // }
		$old_data = mysqli_query($link, "SELECT datetime_start, datetime_end FROM slipping WHERE typeField = '$typeField'");
		...
		$result = mysqli_query($link, "INSERT INTO book (id, firstname, lastname, date, datetime_start, datetime_end, typeField, `status`) VALUES ('$id', '$firstname', '$lastname', '$date', '$datetime_start', '$datetime_end', '$typeField', 'f')");
		if ($result) {
			echo "true";
		} else {
			echo "false";
